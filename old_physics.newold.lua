...
for ei, edge in ipairs(EDGES) do
	...
	local r_cross_n = contact_cx * fny - contact_cy * fnx
	...
	if face_hit or drone.contact_normals[ei] then
		...
		if v_n > 0 then
			...
			vel_x = vel_x + J * fnx
			vel_y = vel_y + J * fny
			local angular_delta = J * r_cross_n
			if v_n > 1.0 then
				drone.angular_velocity = drone.angular_velocity + angular_delta
			end
			if recording then
				table.insert(log_contacts, string.format("  edge%d[%s]: fn=(%.2f,%.2f) rcn=%.2f vn=%.3f J=%.3f contact=(%.2f,%.2f)", ei, contact_source, fnx, fny, r_cross_n, v_n, J, contact_cx, contact_cy))
			end
		end
...
local left_arm_x = new_x - right_dir_x * 7
local left_arm_y = new_y - right_dir_y * 7
local right_arm_x = new_x + right_dir_x * 7
local right_arm_y = new_y + right_dir_y * 7
